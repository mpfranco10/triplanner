...
const updateDocument = function(db, callback, tripId, uid, event) {
    // Get the documents collection
    const collection = db.collection(collectionName);
    // Update document where a is 2, set b equal to 1
    console.log(event);
    collection.updateOne({  "tripId" : tripId, "uid": parseInt(uid) }
...
const removeDocument = function(db, callback, tripId, uid) {
    // Get the documents collection
    const collection = db.collection(collectionName);
    // Delete document where a is 3
    collection.deleteOne({ "tripId" : tripId, "uid": parseInt(uid) }, function(err, result) {
